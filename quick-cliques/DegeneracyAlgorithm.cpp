...
long DegeneracyAlgorithm::Run(list<list<int>> &cliques, vector<bool> const &nodes_status)
{
    return listAllMaximalCliquesDegeneracy(m_AdjacencyList, m_AdjacencyList.size(), nodes_status);
}
...
long DegeneracyAlgorithm::listAllMaximalCliquesDegeneracy(vector<list<int>> const &adjList, int size, vector<bool> const &nodes_status)
{
...
////    clock_t clockStart = clock();
    NeighborListArray** orderingArray = computeDegeneracyOrderArray(adjList, size, nodes_status);
////    clock_t clockEnd = clock();
////    clock_t timeDegeneracyOrder = clockEnd - clockStart;
    ...
    for(i=0;i<size;i++)
    {
        ...
        if (!nodes_status[vertex]) continue;
