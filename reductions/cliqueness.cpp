...
void CliquenessReduction::reduce(size_t const kplex) {
    ...
    for (size_t i = 0; i < _max_clq.size(); i++) {
        if (_max_clq[i] < min_clique_size) _nodes_status[i] = false;
    }
}

void CliquenessReduction::reduce(size_t const clique_size, size_t const kplex) {
